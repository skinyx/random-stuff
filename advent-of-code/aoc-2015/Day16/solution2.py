...
for line in puzzle:
    ...
    for compound in Sue_dict:
        if compound == 'Sue' and compound not in ('cats', 'trees', 'pomeranians', 'goldfish'):
            pass
        elif Sue_dict[compound] == compounds_dict[compound]:
            length -= 1
        if compound in ('cats', 'trees') and Sue_dict[compound] > compounds_dict[compound]:
            length -= 1
        if compound in ('pomeranians', 'goldfish') and Sue_dict[compound] < compounds_dict[compound]:
            length -= 1
